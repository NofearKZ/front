<template>
    ...
    <main class="flex flex-col gap-4 p-12 justify-center min-h-screen items-center bg-gradient-to-b from-gray-200 to-yellow-500 hover:to-blue-500 flex-wrap">
      ...
      <div class="flex gap-4">
        ...
        <label><input class="mx-2" type="radio" value="Vegeta" v-model="selectedpic">Vegeta</label>
        <label><input class="mx-2" type="radio" value="Gogeta" v-model="selectedpic">Gogeta</label>
      </div>
  
      <div class="flex flex-row gap-4 flex-wrap">
  
        <div class="blockcap" v-if="selectedpic == 'Goku'">
          <img :src="pic1.image" class="w-60 h-68 pt-2">
          <p class="text-black text-xl">{{ pic1.name }}</p>
        </div>
  
        <div class="blockcap" v-if="selectedpic == 'Vegeta'">
          <img :src="pic2.image" class="w-60 h-68 pt-2">
          <p class="text-black text-xl">{{ pic2.name }}</p>
        </div>
  
        <div class="blockcap" v-if="selectedpic == 'Gogeta'">
  ...
  <style scoped>
  .blockcap {
    @apply flex flex-col gap-2 border border-blue-500 bg-blue-300 w-72 h-auto items-center hover:border-red-500 hover:border-4 hover:bg-red-300 rounded-xl;
  }
  .blockfor {
    @apply flex flex-col gap-2 border border-blue-500 bg-blue-300 w-72 h-auto items-center hover:border-yellow-500 hover:border-4 hover:bg-yellow-300 rounded-xl;
  }
  </style>
